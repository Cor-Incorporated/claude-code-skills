...
Evidence:|証跡トレーラー
git ls-files|テストはステージ後に回す（2026-09-03 に 2 人が別々に踏んだ形）
MERGED を目視|撤収はマージ確認後（2026-09-03 の削除手戻り）
__pycache__|掃除基準の生成物除外（実測で現れたものだけ）
再生成可能|生成物は「base に無い」に数えない
基準を曲げて|裁量で 1 本だけ消さない
EOF
