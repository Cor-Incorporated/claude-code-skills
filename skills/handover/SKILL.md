...
3. 課金上限: $<実値を書く。既定 $25>
   機械強制（harness H1）は **CODEX_H1_RESTRICTED_MODELS に一致するモデルでのみ**
   掛かる。既定は `sol`。それ以外のモデルでは上限を評価せず、消費は台帳の
   measure / warn 行にだけ残る。2026-09-02 に一律 $5 の上限が通常作業を止めた
   （301 tool call で spend=$5.34、budget_source は
   `unknown-model:gpt-5.6-luna@max-rate`）ため、実績のある最上位モデルへ絞った。
   この欄に書く値は **制限対象モデルに対する上限**である。
...
5. **共通条項を機械的に連結する**（下記「共通条項」節）。
6. テンプレートで文書化し、保存先を確認: 標準は `docs/handover/YYYY-MM-DD-<topic>.md`。

## 共通条項（`common-clauses.md` が正本）

全レーンに共通する注意書きは **`skills/handover/common-clauses.md` の 1 ファイルだけ**に置く。
生成する各ブリーフへは、その中身を **そのまま挿入**する。

```bash
cat skills/handover/common-clauses.md
```

**要約・抜粋・言い換えをしない。** それをした瞬間に不揃いが戻る。
レーン固有の事情は共通条項ではなく、各ブリーフの「所有範囲」欄に書く。

〔2026-08-27 / #91 項目 3〕手書きで各ブリーフへ注意書きを写した結果、
**同じ日に、同じ注意書きを片方のブリーフだけ落とした。**
落とした側が develop 既存の 4 ファイルを独立に再実装し、add/add 競合 4 件・
650 行 対 290 行でブランチ側が全損した。不揃いは記憶に依存するので、
レーン数に比例して漏れる。連結すれば漏れは構造的に消える。

条項を変えるときは **`common-clauses.md` だけ**を直す。
過去のブリーフへ写した文言を個別に追いかけない。
...
## 共通条項
<skills/handover/common-clauses.md の中身をそのまま貼る。要約・抜粋しない>

## 所有範囲
<共通条項に**書かれていない**、このレーン固有の範囲だけを書く>
- 触ってよい: <実パスを列挙。「関連ファイル」のような参照だけの記述は欠陥>
- 触ってはいけない: <実パスを列挙>
- 禁止事項枠: <このレーン固有の禁止。共通の禁止は共通条項にあるので繰り返さない>
